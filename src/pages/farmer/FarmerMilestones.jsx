...
export default function FarmerMilestones() {
  ...
  const [files, setFiles] = useState([]); // Array of { file: File, error: string | null }
  ...
  const loadMilestones = useCallback(async () => {
    ...
    try {
      ...
      // Auto-select first pending milestone if none is selected
      if (!selectedMilestone) {
        const firstPending = data.milestones?.find(m => m.status === "PENDING");
  ...
  const handleFileChange = (e) => {
    ...
    const mapped = selected.map(file => {
      ...
      if (!isJpeg && !isPng && !isPdf) {
        ...
        error = "File size must not exceed 5MB per file.";
      }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!selectedMilestone) {
      setSubmitError("Please select a milestone first.");
      return;
    }
    ...
    if (files.some(f => f.error)) {
      setSubmitError("Please remove invalid files before uploading.");
      return;
    }
                  ...
                  <div key={f.id} style={{ 
                    ...
                    <div style={{ display: "flex", gap: "0.75rem", alignItems: "center", minWidth: 0 }}>
                      ...
                      <div style={{ minWidth: 0, overflow: "hidden" }}>
                        ...
                        {f.error && (
                          <div style={{ fontSize: "0.8rem", color: "#ff5c7a", marginTop: "0.2rem" }}>
                            {f.error}
                          </div>
                        )}
                      </div>
