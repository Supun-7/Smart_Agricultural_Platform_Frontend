...
function StatusBadge({ status }) {
  ...
  return (
    <span style={{
      display: "inline-flex", alignItems: "center", padding: ".35rem .9rem",
...
function DocumentViewer({ label, url }) {
  ...
  return (
    <div style={{ background: "#0e130e", border: "1px solid #1f2b22", borderRadius: 12, overflow: "hidden" }}>
      ...
      {isImage ? (
        <a href={url} target="_blank" rel="noreferrer" title="Open full size" style={{ display: "block" }}>
...
function DecisionModal({ open, loading, title, onCancel, onConfirm }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.75)", display: "grid", placeItems: "center", zIndex: 1000, padding: "1rem" }}>
      <div style={{ width: "100%", maxWidth: 520, background: "#101510", border: "1px solid rgba(255,92,122,.32)", borderRadius: 18, padding: "1.6rem", boxShadow: "0 18px 48px rgba(0,0,0,.4)" }}>
        <h3 style={{ marginTop: 0, color: "var(--text)" }}>{title || "Reject submission"}</h3>
        <p style={{ color: "var(--muted)", lineHeight: 1.5, marginTop: 0 }}>Provide a rejection reason that the user will see. This field is required.</p>
        <textarea
...
export default function KycDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const data = await auditorApi.getKycDetail(token, id);
      setKyc(data);
    } catch (err) {
  ...
  async function handleApprove() {
    ...
    try {
      ...
      setToast({ type: "success", message: "KYC approved successfully." });
      setTimeout(() => navigate(ROUTES.auditorDashboard), 1500);
    } catch (err) {
      setToast({ type: "error", message: err.message || "Failed to approve KYC." });
    } finally {
      setActionLoading("");
    }
  }
  ...
  async function handleReject(reason) {
    ...
    try {
      await auditorApi.rejectKyc(token, id, reason);
      setShowRejectModal(false);
      setToast({ type: "success", message: "KYC rejected." });
      setTimeout(() => navigate(ROUTES.auditorDashboard), 1500);
    } catch (err) {
      setToast({ type: "error", message: err.message || "Failed to reject KYC." });
    } finally {
      setActionLoading("");
    }
  }
  ...
  return (
    <section style={{ minHeight: "100vh", color: "var(--text)", padding: "2rem" }}>
      {toast && (
        <div style={{ position: "fixed", top: 24, right: 24, zIndex: 1100, background: toast.type === "error" ? "#8d1f35" : "#72ad28", color: "#fff", padding: ".85rem 1.1rem", borderRadius: 12, boxShadow: "0 12px 32px rgba(0,0,0,.24)", fontWeight: 700 }}>
          {toast.message}
        </div>
      )}

      <DecisionModal open={showRejectModal} loading={actionLoading === "reject"} title="Reject KYC submission" onCancel={() => setShowRejectModal(false)} onConfirm={handleReject} />

      <header style={{ marginBottom: "1.6rem", display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
        <div>
          <button type="button" onClick={() => navigate(ROUTES.auditorDashboard)} style={{ background: "transparent", border: "1px solid #1f2b22", color: "var(--muted)", borderRadius: 8, padding: ".45rem .9rem", cursor: "pointer", fontSize: ".85rem", marginBottom: ".7rem" }}>
            ← Back to dashboard
          </button>
          <span style={{ display: "block", color: "var(--brand)", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".08em", fontSize: ".8rem" }}>KYC Review</span>
          <h1 style={{ margin: ".3rem 0 0", fontSize: "1.9rem" }}>
            {kyc ? `${kyc.title || ""} ${kyc.firstName || ""} ${kyc.lastName || ""}`.trim() || "KYC Submission" : "Loading..."}
          </h1>
        </div>
      ...
      {loading ? (
        <div style={{ background: "#111611", border: "1px solid #1f2b22", borderRadius: 18, padding: "2rem", color: "var(--muted)" }}>Loading KYC details...</div>
      ) : error ? (
        ...
        <div style={{ display: "grid", gap: 20 }}>
          ...
          <div style={{ background: "#111611", border: "1px solid #1f2b22", borderRadius: 18, padding: "1.4rem" }}>
            <h2 style={{ margin: "0 0 1rem", fontSize: "1.1rem", color: "var(--brand)" }}>Personal Information</h2>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16 }}>
              <InfoRow label="Email" value={kyc.email} />
              <InfoRow label="Title" value={kyc.title} />
              <InfoRow label="First Name" value={kyc.firstName} />
              <InfoRow label="Last Name" value={kyc.lastName} />
              <InfoRow label="Age" value={kyc.age} />
              <InfoRow label="Nationality" value={kyc.nationality} />
              <InfoRow label="Occupation" value={kyc.currentOccupation} />
              <InfoRow label="ID Type" value={kyc.idType} />
              <InfoRow label="ID Number" value={kyc.idNumber} />
              <InfoRow label="Submitted" value={formatDate(kyc.submittedAt)} />
              {kyc.reviewedAt && <InfoRow label="Reviewed" value={formatDate(kyc.reviewedAt)} />}
            </div>
            ...
            {kyc.rejectionReason && (
              <div style={{ marginTop: 14, background: "rgba(255,92,122,.08)", border: "1px solid rgba(255,92,122,.2)", borderRadius: 10, padding: "1rem" }}>
                ...
                <div style={{ color: "#f5d2d9" }}>{kyc.rejectionReason}</div>
              </div>
            )}
          </div>
          ...
          {/* Documents */}
          <div style={{ background: "#111611", border: "1px solid #1f2b22", borderRadius: 18, padding: "1.4rem" }}>
            <h2 style={{ margin: "0 0 1rem", fontSize: "1.1rem", color: "var(--brand)" }}>Identity Documents</h2>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 14 }}>
              <DocumentViewer label="ID Front" url={kyc.idFrontUrl} />
              <DocumentViewer label="ID Back" url={kyc.idBackUrl} />
              <DocumentViewer label="Utility Bill" url={kyc.utilityBillUrl} />
              <DocumentViewer label="Bank Statement" url={kyc.bankStmtUrl} />
            </div>
          </div>
          ...
          {!isPending && (
            <div style={{ background: "#111611", border: "1px solid #1f2b22", borderRadius: 18, padding: "1.2rem", color: "var(--muted)" }}>
              This KYC submission has already been actioned and cannot be reviewed again.
            </div>
          )}
        </div>
      ) : null}
